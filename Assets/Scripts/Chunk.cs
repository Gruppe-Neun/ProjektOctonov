...
public class Chunk : MonoBehaviour
{
	...
	private bool Load() {
        ...
        Block[,,] testBlock = new Block[8, 8, 8];
        for (int z = 0; z < World.chunkSize; z++)
            for (int y = 1; y < World.chunkSize; y++)
                for (int x = 0; x < World.chunkSize; x++) {
                    testBlock[x, y, z] = new Block(Block.BlockType.AIR, new Vector3(x, y, z), this.chunk.gameObject, this);
                }
        for (int z = 0; z < World.chunkSize; z++)
            for (int y = 0; y < 1; y++)
                for (int x = 0; x < World.chunkSize; x++) {
                    testBlock[x, y, z] = new Block(Block.BlockType.DIRT, new Vector3(x, y, z), this.chunk.gameObject, this);
                }


        bd = new BlockData(testBlock);
        return true;
        /*
	...
	private void BuildChunk()
	{
        ...
        for (int z = 0; z < World.chunkSize; z++)
            for (int y = 0; y < 1; y++)
                for (int x = 0; x < World.chunkSize; x++) {
                    chunkData[x, y, z] = new Block(Block.BlockType.DIRT, new Vector3(x, y, z), this.chunk.gameObject, this);
                }
